...
class Mast3rSlamNode(Node):
    def __init__(
        ...
        self.enable_visualizer = self.get_parameter('enable_visualizer').value
        ...
        self.main2viz = new_queue(self.manager, not self.enable_visualizer)
        self.viz2main = new_queue(self.manager, not self.enable_visualizer)

        #dataset = load_dataset(args.dataset)
        #dataset.subsample(config["dataset"]["subsample"])
        # TODO parameterize? These should be hardcoded by some requirements given by asserts in the code
        self.h = 384
        self.w = 512
        ...
        self.backend = mp.Process(target=run_backend, args=(self.config, self.model, self.states, self.keyframes, self.K))
        self.backend.start()

        self.i = 0
        self.fps_timer = time.time()
        self.frames = []
        # TODO add save frames on ctrl-c

    def img_callback(self, img_msg : Image):
        mode = self.states.get_mode()
        msg = try_get_msg(self.viz2main)
        self.last_msg = msg if msg is not None else self.last_msg
        # TODO remove when ctrl-c exit handling is implemented: now we are aborting after the visualizer is closed
        if self.last_msg.is_terminated:
            self.states.set_mode(Mode.TERMINATED)
            #This should stop the module
            exit(0)
            return
        ...
        # TODO check img size
        frame = create_frame(self.i, img, T_WC, img_size=512, device=self.device)
        ...
        # log time
        if self.i % 30 == 0:
            FPS = self.i / (time.time() - self.fps_timer)
            print(f"FPS: {FPS}")
        self.i += 1
        # TODO handle too long keyframes size ???

if __name__ == "__main__":
    use_ros = True
    if use_ros:
        rclpy.init()
        print("Creating ros Mast3rSlamNode")
        node = Mast3rSlamNode()
        print("Node created")
        exe = SingleThreadedExecutor() # MultiThreadedExecutor()
        exe.add_node(node)
        print("Spinning...")
        exe.spin()
        print("Shutting down...")
